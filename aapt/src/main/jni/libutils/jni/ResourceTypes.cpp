...
// made to avoid splitting surrogate pairs.
    ...
    static void deserializeInternal(const void *inData, Res_png_9patch *outData) {
        ...
        if (inData != outData) {
            memmove(&outData->wasDeserialized, patch,
                    4);     // copy  wasDeserialized, numXDivs, numYDivs, numColors
            memmove(&outData->paddingLeft, patch + 12,
                    4);     // copy  wasDeserialized, numXDivs, numYDivs, numColors
        }
    ...
    static status_t
    idmapLookup(const uint32_t *map, size_t sizeBytes, uint32_t key, uint32_t *outValue) {
...
// --------------------------------------------------------------------
    ...
    ResXMLTree::ResXMLTree()
            : ResXMLParser(*this), mError(NO_INIT), mOwnedData(NULL) {
...
// table that defined the package); the ones after are skins on top of it.
            ...
            if (p >= 0) {
                ...
                if (pi != NULL) {
                    ...
                    if (t < pi->numTypes) {
                        ...
                        TABLE_THEME(
                                LOGI("Desired entry index is %ld in avail %d", e, ti.numEntries));
                        if (e < ti.numEntries) {
    ...
    ssize_t ResTable::Theme::resolveAttributeReference(Res_value *inOutValue,
        ...
        if (inOutValue->dataType == Res_value::TYPE_ATTRIBUTE) {
            ...
            TABLE_THEME(LOGI("Resolving attr reference: blockIndex=%d, type=0x%x, data=%p\n",
                             (int) blockIndex, (int) inOutValue->dataType,
                             (void *) inOutValue->data));
    ...
    status_t ResTable::add(const void *data, size_t size, void *cookie,
        ...
        while (((const uint8_t *) chunk) <= (header->dataEnd - sizeof(ResChunk_header)) &&
            ...
            TABLE_NOISY(LOGV("Chunk: type=0x%x, headerSize=0x%x, size=0x%x, pos=%p\n",
                             ...
                             (void *) (((const uint8_t *) chunk) -
                                       ((const uint8_t *) header->header))));
    ...
    ssize_t ResTable::resolveReference(Res_value *value, ssize_t blockIndex,
        ...
        while (blockIndex >= 0 && value->dataType == value->TYPE_REFERENCE
            ...
            TABLE_THEME(LOGI("Resolving reference %p: newIndex=%d, type=0x%x, data=%p\n",
                             (void *) lastRef, (int) newIndex, (int) value->dataType,
                             (void *) value->data));
    ...
    ssize_t ResTable::getBagLocked(uint32_t resID, const bag_entry **outBag,
        ...
        while (ip > 0) {
            ...
            while (pos < count) {
                ...
                TABLE_NOISY(
                        printf("Setting entry #%d %p: block=%d, name=0x%08x, type=%d, data=0x%08x\n",
    ...
    uint32_t ResTable::identifierForName(const char16_t *name, size_t nameLen,
        ...
        for (size_t ig = 0; ig < NG; ig++) {
            ...
            if (ti < 0) {
                TABLE_NOISY(
                        printf("Type not found in package %s\n", String8(group->name).string()));
                continue;
            }
            ...
            if (ei < 0) {
                TABLE_NOISY(
                        printf("Name not found in package %s\n", String8(group->name).string()));
                continue;
            }
    ...
    ssize_t ResTable::getEntry(
        ...
        for (size_t i = 0; i < NT; i++) {
            ...
            TABLE_GETENTRY(
                    LOGI("Match entry 0x%x in type 0x%x (sz 0x%x): imsi:%d/%d=%d/%d lang:%c%c=%c%c cnt:%c%c=%c%c "
        ...
        TABLE_NOISY(aout << "Looking in resource table " << package->header->header
                         ...
                         << (void *) (((const char *) type) -
                                      ((const char *) package->header->header))
    ...
    status_t
    ResTable::createIdmap(const ResTable &overlay, uint32_t originalCrc, uint32_t overlayCrc,
                ...
                for (size_t typeIndex = 0; typeIndex < typeCount; typeIndex++) {
                    ...
                    for (size_t configIndex = 0; configIndex < NTC; configIndex++) {
                        ...
                        if (type->config.inputFlags != 0) {
                            printf(" keyhid=%d",
                                   type->config.inputFlags & ResTable_config::MASK_KEYSHIDDEN);
                            ...
                            printf(" navhid=%d",
                                   type->config.inputFlags & ResTable_config::MASK_NAVHIDDEN);
