...
public:
    ...
    class Item {
    public:
        Item() : isId(false), format(ResTable_map::TYPE_ANY), bagKeyId(0),
                 evaluating(false) { memset(&parsedValue, 0, sizeof(parsedValue)); }
    ...
    public:
        ...
        const DefaultKeyedVector<ConfigDescription, sp<Entry> > &
    ...
    public:
        Type(const String16 &name, const SourcePos &pos)
                : mName(name), mFirstPublicSourcePos(NULL), mPublicIndex(-1), mIndex(-1),
                  mPos(pos) {}
    ...
    public:
        ...
        uint32_t indexOfTypeString(const String16 &s) const {
            return mTypeStringsMapping.valueFor(s);
        }
        ...
        uint32_t indexOfKeyString(const String16 &s) const {
            return mKeyStringsMapping.valueFor(s);
        }
