...
bool AssetManager::addOverlayPath(const String8 &packagePath, int32_t *cookie) {
    ...
    if ((idmap = openAssetFromFileLocked(idmapPath, Asset::ACCESS_BUFFER)) == NULL) {
        LOGW("failed to open idmap file %s\n", idmapPath.string());
        return false;
    }
    ...
    if (!ResTable::getIdmapInfo(idmap->getBuffer(false), idmap->getLength(),
        ...
        LOGW("failed to read idmap file %s\n", idmapPath.string());
        delete idmap;
        return false;
    }
    delete idmap;
    ...
    if (overlayPath != packagePath) {
        LOGW("idmap file %s inconcistent: expected path %s does not match actual path %s\n",
              idmapPath.string(), packagePath.string(), overlayPath.string());
        return false;
    }
    if (access(targetPath.string(), R_OK) != 0) {
        LOGW("failed to access file %s: %s\n", targetPath.string(), strerror(errno));
        return false;
    }
    ...
    oap.idmap = idmapPath;
#if 0
    LOGD("Overlay added: targetPath=%s overlayPath=%s idmapPath=%s\n",
...
bool AssetManager::createIdmap(const char *targetApkPath, const char *overlayApkPath,
                               uint32_t targetCrc, uint32_t overlayCrc, uint32_t **outData,
                               size_t *outSize) {
    AutoMutex _l(mLock);
    ...
    return tables[0].createIdmap(tables[1], targetCrc, overlayCrc,
                                 targetApkPath, overlayApkPath, (void **) outData, outSize) ==
           NO_ERROR;
}
...
Asset *AssetManager::openNonAsset(const char *fileName, AccessMode mode, int32_t *outCookie) {
    ...
    while (i > 0) {
        ...
        LOGV("Looking for non-asset '%s' in '%s'\n", fileName,
              mAssetPaths.itemAt(i).path.string());
...
//    ATRACE_NAME(ap.path.string());
    ...
    if ((ass != NULL || sharedRes != NULL) && ass != kExcludedAsset) {
        ...
        LOGV("Installing empty resources in to table %p\n", mResources);
...
void AssetManager::addSystemOverlays(const char *pathOverlaysList,
                                     const String8 &targetPackagePath, ResTable *sharedRes,
                                     size_t offset) const {
...
Asset *AssetManager::openInLocaleVendorLocked(const char *fileName, AccessMode mode,
                                              const asset_path &ap, const char *locale,
                                              const char *vendor) {
    Asset *pAsset = NULL;
...
Asset *AssetManager::openAssetFromZipLocked(const ZipFileRO *pZipFile,
                                            const ZipEntryRO entry, AccessMode mode,
                                            const String8 &entryName) {
    Asset *pAsset = NULL;
...
bool AssetManager::scanAndMergeDirLocked(SortedVector<AssetDir::FileInfo> *pMergedInfo,
                                         const asset_path &ap, const char *rootDir,
                                         const char *dirName) {
...
bool AssetManager::scanAndMergeZipLocked(SortedVector<AssetDir::FileInfo> *pMergedInfo,
    ...
    if (!pZip->startIteration(&iterationCookie, dirName.string(), NULL)) {
        LOGW("ZipFileRO::startIteration returned false");
        return false;
    }
