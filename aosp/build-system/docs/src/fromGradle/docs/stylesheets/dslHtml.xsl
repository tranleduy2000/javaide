...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match="chapter" mode="object.title.markup.textonly">
        ...
        <xsl:text>-</xsl:text>
        <xsl:apply-templates mode="object.title.markup.textonly" select="/book" />
    </xsl:template>
                ...
                <div class="sidebar">
                    <ul>
                        ...
                        <xsl:apply-templates mode="sidebar"
                            select="/book/section/table[@role = 'dslTypes']" />
                    </ul>
                </div>
                <div class="content">
                    ...
                    <div id="copyright">
                        ...
                        For details and restrictions, see the <a
                        href="http://developer.android.com/license.html">Content License</a>.
                    </div>
                </div>
    ...
    <xsl:template match="book" mode="sidebar">
        ...
        <ul class='sections'>
            <xsl:apply-templates mode="sidebar.link" select="section" />
        </ul>
    </xsl:template>
    ...
    <xsl:template match="chapter" mode="sidebar">
        ...
        <ul class='sections'>
            <xsl:apply-templates mode="sidebar.link" select="section[table]" />
        </ul>
    </xsl:template>
    ...
    <xsl:template match="table" mode="sidebar">
        ...
        <xsl:apply-templates mode="sidebar" select="tr/td[1]" />
    </xsl:template>
    ...
    <xsl:template match="segmentedlist">
        <div>
            ...
            <xsl:call-template name="anchor" />
            <table>
                <xsl:apply-templates mode="seglist.table" select="seglistitem/seg" />
            </table>
        </div>
    </xsl:template>
    ...
    <xsl:template match="section[@role='detail']/segmentedlist">
        <div>
            ...
            <xsl:call-template name="anchor" />
            <dl>
                <xsl:apply-templates mode="seglist.list" select="seglistitem/seg" />
            </dl>
        </div>
    </xsl:template>
    ...
    <xsl:template match="seg" mode="seglist.table">
        ...
        <tr>
            <th><xsl:apply-templates mode="segtitle-in-seg"
                select="$segtitles[$segnum=position()]" />:
            </th>
            <td>
                <xsl:apply-templates />
            </td>
    ...
    <xsl:template match="seg" mode="seglist.list">
        ...
        <xsl:variable name="seglist" select="ancestor::segmentedlist" />
        <xsl:variable name="segtitles" select="$seglist/segtitle" />
        <dt><xsl:apply-templates mode="segtitle-in-seg" select="$segtitles[$segnum=position()]" />:
        </dt>
        <dd>
            <xsl:apply-templates />
        </dd>
