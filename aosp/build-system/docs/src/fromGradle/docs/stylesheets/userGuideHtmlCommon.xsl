...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xslthl="http://xslthl.sf.net"
    ...
    <xsl:template name="customXref">
        ...
        <xsl:param name="content">
            <xsl:apply-templates mode="object.title.markup" select="$target" />
        </xsl:param>
        <a>
            ...
            <xsl:attribute name="title">
                <xsl:apply-templates mode="object.title.markup.textonly" select="$target" />
            </xsl:attribute>
    ...
    <xsl:template match="book|chapter|appendix|section|tip|note"
        mode="html.title.attribute"></xsl:template>
    ...
    <xsl:template match="table">
        <xsl:param name="class">
            <xsl:apply-templates mode="class.value" select="." />
        </xsl:param>
        <div class="{$class}">
            <xsl:if test="title">
    ...
    <xsl:template match="title" mode="htmlTable"></xsl:template>
