...
public class Paths {
    ...
    /*private Path computeAndroidClasspath() {
        ...
        path.addDirectory(new File(envs.get("HOME")+"/classes"),true);
...
//        path.addDirectory(new File("/sdcard"),true);
    ...
    private Path computeBootClassPath() {
        ...
        if (!bootclasspath.equals(defaultClasspath)) {
...
//        return computeAndroidClasspath();
        Map<String, String> envs = System.getenv();
//        path.addFiles(envs.get("CLASSPATH"));

        String cp = options.get(CLASSPATH);

        // CLASSPATH environment variable when run from `javac'.
        if (cp == null) cp = envs.get("CLASSPATH");
        ...
        String vb = options.get(VERBOSE);
        if (vb != null) {
            System.out.println("USER CLASS PATH : " + cp);
        }
    ...
    private Path computeSourcePath() {
        ...
        String vb = options.get(VERBOSE);
        if (vb != null) {
            System.out.println("SOURCE CLASS PATH : " + sourcePathArg);
        }
