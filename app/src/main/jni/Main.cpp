...
int handleCommand(Bundle *bundle) {
    ...
    switch (bundle->getCommand()) {
        ...
        case kCommandPackage:
            return doPackage(bundle);
        case kCommandCrunch:
            return doCrunch(bundle);
        case kCommandSingleCrunch:
            return doSingleCrunch(bundle);
        case kCommandDaemon:
            return runInDaemonMode(bundle);
        default:
            ...
            switch (*cp) {
                ...
                case '-':
                    if (strcmp(cp, "-debug-mode") == 0) {
                        ...
                        if (!argc) {
                            fprintf(stderr,
                                    "ERROR: No argument supplied for '--preferred-density' option\n");
                            wantUsage = true;
                            goto bail;
                        }
                        ...
                        if (!argc) {
                            fprintf(stderr,
                                    "ERROR: No argument supplied for '--feature-of' option\n");
                            wantUsage = true;
                            goto bail;
                        }
                        ...
                        if (!argc) {
                            fprintf(stderr,
                                    "ERROR: No argument supplied for '--feature-after' option\n");
                            wantUsage = true;
                            goto bail;
                        }
