...
public class DynamicLayout extends Layout
{
     ...
     * the text is changed.
     */
    public DynamicLayout(LayoutContext layoutContext, CharSequence base,
                         TextPaint paint,
     ...
     */
    public DynamicLayout(LayoutContext layoutContext, CharSequence base, CharSequence display,
                         TextPaint paint,
                         int width, Alignment align,
                         float spacingmult, float spacingadd,
                         boolean includepad) {
...
//        }
        ...
        start[TOP] = 0;
        start[DESCENT] = desc;
        start[LINE_BREAK] = 1; //jec: default has newline
    ...
    private void reflow(CharSequence s, int where, int before, int after) {
        ...
        if (reflowed == null) {
            reflowed = new StaticLayout(layoutContext, null);
        } else {
            reflowed.prepare();
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            ints[LINE_BREAK] = reflowed.isRealNewLine(i) ? 1 : 0;
    ...
    public boolean isRealNewLine(int line) {
        return mInts.getValue(line, LINE_BREAK) == 1;
    }

    @Override
    public int getRealLine(int line) {
        if(!isRealNewLine(line))
            return 0;
        int realLine = 0;
