 ...
 * the screen.
 * <p>For text that will be edited, use a {@link android.text.DynamicLayout},
...
//    }
     ...
     */
    public static float getDesiredWidth(LayoutContext layoutContext, CharSequence source,
                                        TextPaint paint) {
        return getDesiredWidth(layoutContext, source, 0, source.length(), paint);
    }
    ...
    public static float getDesiredWidth(LayoutContext layoutContext, CharSequence source,
        ...
        for (int i = start; i <= end; i = next) {
            ...
            float w = measurePara(layoutContext, paint, source, i, next);
     ...
     */
    protected Layout(LayoutContext layoutContext, CharSequence text, TextPaint paint,
                     int width, Alignment align,
                     float spacingMult, float spacingAdd) {
        this(layoutContext, text, paint, width, align, TextDirectionHeuristics.FIRSTSTRONG_LTR,
...
//            }
            ...
            if (directions == DIRS_ALL_LEFT_TO_RIGHT && !mSpannedText && !hasTabOrEmoji) {
                ...
                tl.set(layoutContext, paint, buf, start, end, dir, directions, hasTabOrEmoji, tabStops);
    ...
    private float getHorizontal(int offset, boolean trailing, int line, boolean clamped) {
        ...
        TextLine tl = TextLine.obtain();
        tl.set(layoutContext, mPaint, mText, start, end, dir, directions, hasTabOrEmoji, tabStops);
    ...
    private float getLineExtent(int line, boolean full) {
        ...
        TextLine tl = TextLine.obtain();
        tl.set(layoutContext, mPaint, mText, start, end, dir, directions, hasTabsOrEmoji, tabStops);
    ...
    private int getOffsetToLeftRightOf(int caret, boolean toLeft) {
        ...
        tl.set(layoutContext, mPaint, mText, lineStart, lineEnd, lineDir, directions, false, null);
    ...
    public void getCursorPath(int point, Path dest,
        ...
        h2 += layoutContext.cursorThickness;

        int caps = TextKeyListener.getMetaState(editingBuffer, TextKeyListener.META_SHIFT_ON) |
                   TextKeyListener.getMetaState(editingBuffer, TextKeyListenerCompat.META_SELECTING);
...
//        return margin;
    ...
    static float measurePara(LayoutContext layoutContext, TextPaint paint, CharSequence text, int start, int end) {
        ...
        try {
            ...
            tl.set(layoutContext, paint, text, start, end, dir, directions, hasTabs, tabStops);
    ...
    private static final char TAB_SIGN= '\u00bb'; //\t  or \u203A
    private static final char CARRIAGE_RETURN_SIGN= '\u00a4'; // \r
