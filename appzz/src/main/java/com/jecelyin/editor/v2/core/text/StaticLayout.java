 ...
 * is laid out.  Use {@link android.text.DynamicLayout} for text that may change.
...
//    }
     ...
     * @hide
     */
    public StaticLayout(LayoutContext layoutContext, CharSequence source, int bufstart, int bufend,
            TextPaint paint, int outerwidth,
            Alignment align, TextDirectionHeuristic textDir,
            float spacingmult, float spacingadd,
            boolean includepad) {
        this(layoutContext, source, bufstart, bufend, paint, outerwidth, align, textDir,
    ...
    public StaticLayout(LayoutContext layoutContext, CharSequence source, int bufstart, int bufend,
        ...
        this(layoutContext, source, bufstart, bufend, paint, outerwidth, align,
    ...
    public StaticLayout(LayoutContext layoutContext, CharSequence source, int bufstart, int bufend,
        ...
        super(layoutContext, /*(ellipsize == null)
                ?*/ source
...
//        final String localeLanguageTag = paint.getTextLocale().toLanguageTag();
        final String localeLanguageTag;
        if(Build.VERSION.SDK_INT >= 21) {
            localeLanguageTag = paint.getTextLocale().toLanguageTag();
        } else {
...
//            paraEnd = TextUtils.indexOf(source, CHAR_NEW_LINE, paraStart, bufEnd);
            paraEnd = TextUtils.indexOfNewLineChar(source, paraStart, bufEnd);  //支持\r换行
            if (paraEnd < 0)
                paraEnd = bufEnd;
            else
                paraEnd++;
            ...
            boolean newline = here == paraStart;
            ...
            for (int spanStart = paraStart, spanEnd; spanStart < paraEnd; spanStart = spanEnd) {
                ...
                for (int j = spanStart; j < spanEnd; j++) {
                    ...
                    if (c == CHAR_NEW_LINE || c == CHAR_NEW_LINE_CR) {
...
//                        }
                    ...
                    if (w <= width || isSpaceOrTab) {
                        ...
                        v = out(source, here, endPos,
                                ...
                                currentTextWidth, paint, moreChars, newline);
            ...
            if (paraEnd != here && mLineCount < mMaximumVisibleLineCount) {
                ...
                v = out(source,
                        ...
                        ellipsizedWidth,*/ w, paint, paraEnd != bufEnd, newline);
            }
        ...
        if ((bufEnd == bufStart || source.charAt(bufEnd - 1) == CHAR_NEW_LINE || source.charAt(bufEnd - 1) == CHAR_NEW_LINE_CR) &&
            ...
            v = out(source,
                    ...
                    ellipsizedWidth,*/ 0, paint, false, true);
        }
    }
...
//                      TextPaint paint, boolean moreChars) {
                      TextPaint paint, boolean moreChars, boolean newline) {
        int j = mLineCount;
        int off = j * mColumns;
        ...
        lines[off + LINE_BREAK] = newline ? 1 : 0;
...
//        }
    ...
    public boolean isRealNewLine(int line) {
        return mLines[mColumns * line + LINE_BREAK] == 1;
    }

    @Override
    public int getRealLine(int line) {
        if(mLines[mColumns * line + LINE_BREAK] == 0)
            return 0;
        int realLine = 0;
        for(int i=0; i<=line; i++) {
            if(mLines[mColumns * i + LINE_BREAK] == 1)
                realLine++;
        }
        return realLine;
    }
...
//    private static native int[] nLineBreakOpportunities(String locale, char[] text, int length, int[] recycle);
    /*package*/ static int[] nLineBreakOpportunities(String locale, char[] text, int length,
                                                     int[] recycle) {
//        BreakIterator iterator = BreakIterator.getLineInstance(new ULocale(locale).toLocale()); // TODO: 15/9/1  ICU
        BreakIterator iterator = BreakIterator.getLineInstance(new Locale(locale));
