...
class TextLine {
     ...
     */
    void set(LayoutContext layoutContext, TextPaint paint, CharSequence text, int start, int limit, int dir,
            Layout.Directions directions, boolean hasTabs, Layout.TabStops tabStops) {
                ...
                if (j == runLimit || codept == '\t' || bm != null) {
                    ...
                    if (codept == '\t') {
                        ...
                        if (layoutContext.isShowWhiteSpace) {
                            ...
                            wp.setStrokeWidth(2f);
                            wp.setStyle(Paint.Style.FILL);
                            c.drawLine(x2 + 3, y - descent - 4f, h - 3, y - descent - 4f, wp);
                        }
...
//        int dir = runIsRtl ? Paint.DIRECTION_RTL : Paint.DIRECTION_LTR;
        int dir = runIsRtl ? TextPaintCompat.DIRECTION_RTL : TextPaintCompat.DIRECTION_LTR;
//        int cursorOpt = after ? Paint.CURSOR_AFTER : Paint.CURSOR_BEFORE;
        int cursorOpt = after ? TextPaintCompat.CURSOR_AFTER : TextPaintCompat.CURSOR_BEFORE;
        if (mCharsValid) {
//            return wp.getTextRunCursor(mChars, spanStart, spanLimit - spanStart,
//                    dir, offset, cursorOpt);
            return TextPaintCompat.getTextRunCursor(wp, mChars, spanStart, spanLimit - spanStart,
                    dir, offset, cursorOpt);
        } else {
...
//                        contextStart, contextLen, runIsRtl, null, 0);
                ret = TextPaintCompat.getTextRunAdvances(wp, mChars, start, runLen,
                        contextStart, contextLen, runIsRtl, null, 0);
            } else {
                int delta = mStart;
...
//                        runIsRtl, null, 0);
                ret = TextPaintCompat.getTextRunAdvances(wp, mText, delta + start,
                        delta + end, delta + contextStart, delta + contextEnd,
                        runIsRtl, null, 0);
            }
        }
...
//            if (wp.underlineColor != 0) {
            if (TextPaintCompat.getUnderlineColor(wp) != 0) {
...
//                wp.setColor(wp.underlineColor);
                wp.setColor(TextPaintCompat.getUnderlineColor(wp));
//                c.drawRect(x, underlineTop, x + ret, underlineTop + wp.underlineThickness, wp);
                c.drawRect(x, underlineTop, x + ret, underlineTop + TextPaintCompat.getUnderlineThickness(wp), wp);
...
//                    x, y, runIsRtl, wp);
            drawTextRun(c, mChars, start, count, contextStart, contextCount,
                    x, y, runIsRtl, wp);
        } else {
            int delta = mStart;
//            c.drawTextRun(mText, delta + start, delta + end,
//                    delta + contextStart, delta + contextEnd, x, y, runIsRtl, wp);
            drawTextRun(c, mText, delta + start, delta + end,
                    delta + contextStart, delta + contextEnd, x, y, runIsRtl, wp);
        }
    }

    private void drawTextRun(Canvas c, @NonNull CharSequence text, int start, int end, int contextStart,
                                   int contextEnd, float x, float y, boolean isRtl, @NonNull Paint paint) {
        int contextLen = contextEnd - contextStart;
        int len = end - start;
        char[] buf = TemporaryBuffer.obtain(contextLen);
        TextUtils.getChars(text, contextStart, contextEnd, buf, 0);
        drawTextRun(c, buf, start - contextStart, len, 0, contextLen, x, y, isRtl, paint);
        TemporaryBuffer.recycle(buf);
    }

    private void drawTextRun(Canvas c, @NonNull char[] text, int index, int count, int contextIndex,
                                   int contextCount, float x, float y, boolean isRtl, @NonNull Paint paint) {
        if (!layoutContext.isShowWhiteSpace) {
            CanvasCompat.drawTextRun(c, text, index, count, contextIndex, contextCount, x, y, isRtl, paint);
            return;
        }
        int start = index;
        int end = index + count;
        ...
        for (int next = start; next < end; start = next) {
            space = text[start] == ' ';
            next = getNextTransition(text, next, end, space);

            if (space) {
                paint.setColor(layoutContext.whiteSpaceColor);
            } else {
    ...
    float nextTab(float h) {
        ...
        return Layout.TabStops.nextDefaultStop(h, Layout.TAB_INCREMENT);
    }
