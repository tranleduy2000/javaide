...
public class BuildTask extends ProgressTask {
    ...
    private void runAapt() throws Exception {
        ...
        int exitCode = aapt.fnExecute("aapt p -f -v -M " + S.xmlMan.getPath() + " -F " + S.ap_Resources.getPath()
                + " -I " + S.jarAndroid.getPath()
                + " -A " + S.dirAssets.getPath() + " -S " + S.dirRes.getPath()
                + " -J " + S.dirGen.getPath());
    ...
    private void compileJava() throws Exception {
        ...
        org.eclipse.jdt.core.compiler.batch.BatchCompiler.compile(
                new String[]{"-1.5", "-showversion", "-verbose", "-deprecation",
                        "-bootclasspath", S.getJavaBootClassPath(),
                        "-cp", S.getJavaClassPath(),
                        "-d", S.dirClasses.getPath(),
    ...
    private void buildApk() throws Exception {
        ...
        ApkBuilder apkbuilder = new ApkBuilder(S.apkUnsigned,
                S.ap_Resources, S.dexClasses, privateKey, x509Cert, System.out);
        ...
        Log.d(TAG, "buildApk: " + new S().toString());
