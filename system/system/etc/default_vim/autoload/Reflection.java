...
class Reflection {
	...
	//System.out.println("collectClassPath");
	...
	while (st.hasMoreTokens()) {
	    ...
	    //System.out.println("htc classpath found : "+path);
	    ...
	    if (path.endsWith(".jar") || path.endsWith(".zip")){
			...
			//System.out.println("htc add classpath : "+f.toString());
		} else {
			if (f.isDirectory()){
		    	htClasspath.put(f.toString(), "");
				//System.out.println("htc add classpath : "+f.toString());
	...
	//System.out.println("htc classpath : "+htClasspath.size());
	...
	private static Hashtable mClassCounter = new Hashtable();
    private static void putClassInfo(Hashtable map, Class clazz) {
		...
		//We allready scanned this one
		if(mClassCounter.containsKey(name)){
			return;
		}
		
		//Otherwise add it..
		mClassCounter.put(name,"");
